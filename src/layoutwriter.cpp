...
//	         "mesh.z = [mesh.z, " << extrem_pos_zmin << ", " << extrem_pos_zmax << "];\n"
	...
	f_out << "%%%% VARIABLES\n"
	         ...
	         "\tfunit_name = 'MHz';\n"
	         "endif\n"
	         "freq = linspace(fstart, fstop, 501);\n";
	vector<pair<unsigned int, shared_ptr<Element>>> ports;
	for(shared_ptr<Element> it : block.elements) {
		if(it->getType()=="Pac") {
			f_out << "port{" << it->getN() << "} = calcPort(port{" << it->getN() << "}, Sim_Path, freq);\n";
	...
	for(pair<unsigned int, shared_ptr<Element>> it : ports) {
		for(pair<unsigned int, shared_ptr<Element>> ut : ports) {
			f_out << "s" << it.first << ut.first << " = port{" << it.first << "}.uf.ref ./ port{" << ut.first << "}.uf.inc;\n";
			}
		}
	f_out << "\n";
	...
	for(unsigned int i=1;i<=ports.size();i++) {
		...
		for(unsigned int j=1;j<=ports.size();j++) {
			...
			f_out << "spara(" << i << "," << j << ",:) = s" << ports[i-1].first << ports[j-1].first << ";\n";
			}
		}
	f_out << "write_touchstone('s', freq, spara, '" << name << ".s" << ports.size() << "p');\n"
	         "% Append header to touchstone file\n"
	         "fd = fopen('" << name << ".s" << ports.size() << "p', 'r');\n"
	         "tmp = fread(fd);\n"
	         "fclose(fd);\n"
	         "fd = fopen('" << name << ".s" << ports.size() << "p', 'wt');\n"
	...
	f_out << "%%%% PLOT S PARAMETERS\n"
	         "figure;\n"
	         "plot(freq/funit, 20*log10(abs(s" << ports.front().first << ports.front().first << ")), "
	         "'" << colors[color++%color_max] << "-;s" << ports.front().first << ports.front().first << ";', 'Linewidth', 2);\n";
	for(pair<unsigned int, shared_ptr<Element>> it : ports) {
		for(pair<unsigned int, shared_ptr<Element>> ut : ports) {
			...
			f_out << "hold on;\n"
			         "plot(freq/funit, 20*log10(abs(s" << it.first << ut.first << ")), "
			         "'" << colors[color++%color_max] << "-;s" << it.first << ut.first << ";', 'Linewidth', 2);\n";
			}
		}
	f_out << "grid on;\n"
