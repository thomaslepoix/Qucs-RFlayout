...
void LayoutWriter::write_m(Block& block, std::ofstream& f_out, long double const offset_x, long double const offset_y, std::string const& name) {
	...
	f_out << "arg_list = argv();\n"
	         ...
	         "\t\tdisp(\"\\t--nf2ff-frames       Number of 3D frames. Default is 0 : only 1 frame on the most radiating frequency.\");\n"
...
//	         "mesh.z = [mesh.z, " << extrem_pos_zmin << ", " << extrem_pos_zmax << "];\n"
	...
	f_out << "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n"
	         ...
	         "%   'gif':                 - Merge all frames in an animated gif.\n"
	         ...
	         "%                          - Default: 'jet'.\n"
	         "%   'show_each':           - Show each frame in its figure window.\n"
	         "%   'no_print':            - Do not save frames/gif on disk.\n"
	         "%   'format' <str>:        - Frame format.\n"
	         "%                          - You probably must not override it.\n"
	         "%                          - Default: 'png'.\n"
	         "%\n"
	         "% example:\n"
	         "%   plotFF3D_frames(nf2ff, [name, '-ff3d-vm'], 'gif', ...\n"
	         ...
	         "\t\tshow_each = true;\n"
	         "\telseif (strcmp(varargin{n}, 'no_print') == 1);\n"
	         "\t\tprint_arg = false;\n"
	...
	f_out << "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n"
	         ...
	         "%   f_ind:     Index in 'freq' of the frequency to put a marker on.\n"
	         "%\n"
	         "% variable input:\n"
	         ...
	         "h = plot(s(f_ind),['o;', ...\n"
	         "\tname, ' @ ', num2str(freq(f_ind)/1e6, ffmt), ' MHz', ...\n"
	         "\t\"\\n\", num2str(20*log10(abs(s(f_ind))), '%.1f'), 'dB ', ...\n"
	         "%\tnum2str(s(f_ind), '%4.2f'), ' Ω', ...\n"
	         "\tz_str, ...\n"
	         "\t';']);\n"
	...
	for(pair<unsigned int, shared_ptr<Element>> it : ports) {
		...
		for(pair<unsigned int, shared_ptr<Element>> ut : ports) {
			f_out << "if first == true\n"
			         "\tfirst = false;\n"
			         "\th = plotSmith(s" << ut.first << it.first << ", 's" << ut.first << it.first << "', freq, f_res_ind);\n"
			         "else\n"
	...
	f_out << "legend('Location', 'northeastoutside');\n"
	         "drawnow;\n"
	         "print([name, '-smith', graphics_format]);\n"
	         "\n";
...
//	f_out << "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n"
	...
	f_out << "f_res = freq(f_res_ind);\n"
	         "% NF2FF center should be placed at the center of the radiating element.\n"
	         "center = [(max(mesh.x)-min(mesh.x))/2, (max(mesh.y)-min(mesh.y))/2, 0];\n"
	         "\n";
	...
	f_out << "%%%% ELEVATION\n"
	         ...
	         "h = polarFF(nf2ff, 'xaxis', 'theta', 'param', [1, 2], 'normalize', 1, 'xtics', 10);\n"
	         "set(h, 'Linewidth', 2);\n"
	         "drawnow;\n"
	         "print([name, '-ffp-elev-dbn', graphics_format]);\n"
	         "\n"
	         "% Polar dBi\n"
	         "figure;\n"
	         "set(gca, 'ColorOrder', color_order_polar, 'NextPlot', 'replacechildren');\n"
	         "h = polarFF(nf2ff, 'xaxis', 'theta', 'param', [1, 2], 'logscale', [-20, 10], 'xtics', 10);\n"
	         "set(h, 'Linewidth', 2);\n"
	         "drawnow;\n"
	         "print([name, '-ffp-elev-dbi', graphics_format]);\n"
	         "\n"
	         "% Rect dBi\n"
	         "figure;\n"
	         "set(gca, 'ColorOrder', color_order, 'NextPlot', 'replacechildren');\n"
	         "h = plotFFdB(nf2ff, 'xaxis', 'theta', 'param', [1, 2]);\n"
	         "set(h, 'Linewidth', 2);\n"
	         "drawnow;\n"
	         "print([name, '-ffr-elev-dbi', graphics_format]);\n"
	         "\n";
	...
	f_out << "%%%% AZIMUTH\n"
	         ...
	         "figure;\n"
	         "set(gca, 'ColorOrder', color_order_polar, 'NextPlot', 'replacechildren');\n"
	         "h = polarFF(nf2ff, 'xaxis', 'phi', 'param', [1, 2, 3], 'normalize', 1, 'xtics', 10);\n"
	         "set(h, 'Linewidth', 2);\n"
	         "drawnow;\n"
	         "print([name, '-ffp-azim-dbn', graphics_format]);\n"
	         "\n"
	         "% Polar dBi\n"
	         "figure;\n"
	         "set(gca, 'ColorOrder', color_order_polar, 'NextPlot', 'replacechildren');\n"
	         "h = polarFF(nf2ff, 'xaxis', 'phi', 'param', [1, 2, 3], 'logscale', [-20, 10], 'xtics', 10);\n"
	         "set(h, 'Linewidth', 2);\n"
	         "drawnow;\n"
	         "print([name, '-ffp-azim-dbi', graphics_format]);\n"
	         "\n"
	         "% Rect dBi\n"
	         "figure;\n"
	         "set(gca, 'ColorOrder', color_order, 'NextPlot', 'replacechildren');\n"
	         "h = plotFFdB(nf2ff, 'xaxis', 'phi', 'param', [1, 2, 3]);\n"
	         "set(h, 'Linewidth', 2);\n"
	         "drawnow;\n"
	         "print([name, '-ffr-azim-dbi', graphics_format]);\n"
	         "\n";
	...
	f_out << "%%%% 3D\n"
	         ...
	         "theta_range = [0:5:180];\n"
	         "phi_range = [-180:5:180];\n"
	         "if flag_nf2ff_frames\n"
	         "\tfreqs = [fstart:(fstop-fstart) / (flag_nf2ff_frames-1):fstop];\n"
	         "else\n"
	         "\tfreqs = f_res;\n"
	         "endif\n"
	         "nf2ff = CalcNF2FF(nf2ff, Sim_Path, freqs, theta_range*pi/180, phi_range*pi/180, ...\n"
	         "\t'Center', center, ...\n"
...
/*	         "% V/m\n"
	         ...
	         "plotFF3D(nf2ff);\n"
	         "drawnow;\n"
	         "print([name, '-ff3d-vm', '.png']);\n"
	         "\n"
	         "% dB\n"
	         "figure;\n"
	         "plotFF3D(nf2ff, 'logscale', -20);\n"
	         "drawnow;\n"
	         "print([name, '-ff3d-db', '.png']);\n"
	         "\n"
*/	         "% Dump\n"
	         "if flag_dump\n"
	         "E_far_normalized = nf2ff.E_norm{1} / max(nf2ff.E_norm{1}(:)) * nf2ff.Dmax(1);\n"
	         "DumpFF2VTK([Sim_Path '/ff.vtk'], E_far_normalized, theta_range, phi_range, 'scale', unit * 10);\n"
	         "endif % flag_dump\n"
	         "\n";
