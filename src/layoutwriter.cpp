...
//******************************************************************************
int LayoutWriter::write(Block const& block, long double const offset_x, long double const offset_y, filesystem::path const& n_out, string const& name, vector<string>* out_names) const {
...
//******************************************************************************
void LayoutWriter::write_kicad_pcb(Block const& block, ofstream& f_out, long double const offset_x, long double const offset_y, string const& name) const {
	string type;
...
//******************************************************************************
void LayoutWriter::write_kicad_mod(Block const& block, ofstream& f_out, long double const offset_x, long double const offset_y, string const& name) const {
	string type;
	string label;
	smatch match;
...
//******************************************************************************
void LayoutWriter::write_lht(Block const& block, ofstream& f_out, long double const offset_x, long double const offset_y, string const& name) const {
	string type;
	unsigned int n=0;
...
//******************************************************************************
void LayoutWriter::write_m(Block const& block, std::ofstream& f_out, long double const offset_x, long double const offset_y, std::string const& name) const {
	string type;
	string label;
...
//******************************************************************************
void LayoutWriter::write_svg(Block const& block, ofstream& f_out, long double const offset_x, long double const offset_y, string const& name) const {
	static constexpr char const* px_to_mm_factor = "3.779527559";

	f_out << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
	         "<!-- Generated by Qucs-RFlayout from : " << name << ".sch -->\n"
	         "<svg xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\" "
	         // TODO how to fix size ??
	         "preserveAspectRatio=\"xMidYMid meet\" "
	         "viewBox=\"" << (block.margin_boundary[XMIN]+offset_x) * stof(px_to_mm_factor) << " "
	                      << (block.margin_boundary[YMIN]+offset_y) * stof(px_to_mm_factor) << " "
	                      << (block.margin_boundary[XMAX]+offset_x) * stof(px_to_mm_factor) << " "
	                      << (block.margin_boundary[YMAX]+offset_y) * stof(px_to_mm_factor) << "\" "
	         "width=\""  << block.margin_boundary[XMAX]-block.margin_boundary[XMIN] << "mm\" "
	         "height=\"" << block.margin_boundary[YMAX]-block.margin_boundary[YMIN] << "mm\">\n"
	         "  <g id=\"" << name << "\" transform=\"scale(" << px_to_mm_factor << ")\">\n";
	...
	for(shared_ptr<Element> it : block.elements) {
		...
		if(type=="SUBST" || type=="MGAP" || type=="MOPEN" || type=="MSTEP") {
			...
			f_out << "    <rect "
			         ...
			         "y=\"" << it->getY()+offset_y - ((pac->is_size_set ? it->getL() : 0.01) / 2) << "\" "
			         "width=\"" << (pac->is_size_set ? it->getW() : 0.01) << "\" "
			         "height=\"" << (pac->is_size_set ? it->getL() : 0.01) << "\" "
		       ...
		       || type=="MRSTUB"
		       || type=="MTEE") {///////////////////////////////////////////////
			f_out << "    <path id=\"" << it->getLabel() << "\" d=\"";
			for(int i=0; i<it->getNpoint(); i++) {
				f_out << (i==0 ? "M" : "L")
				      << it->getP(i, X, R, ABS)+offset_x << " "
				      << it->getP(i, Y, R, ABS)+offset_y << " ";
				}
			...
			f_out << "    <path id=\"" << it->getLabel() << "\" d=\"";
			for(int i=0;i<it->getNpoint();i++) {
				f_out << (i==0 || i==it->getNpoint()/2 ? "M" : "L")
				      << it->getP(i, X, R, ABS)+offset_x << " "
				      << it->getP(i, Y, R, ABS)+offset_y << " ";
				}
			f_out << "Z\" fill=\"black\" stroke=\"none\"/>\n";
